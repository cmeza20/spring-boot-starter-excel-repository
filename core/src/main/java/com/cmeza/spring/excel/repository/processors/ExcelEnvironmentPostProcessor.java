package com.cmeza.spring.excel.repository.processors;
...
public class ExcelEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        try {
            application.addInitializers(ctx -> log.replayTo(ExcelEnvironmentPostProcessor.class));
            String path = environment.getProperty("spring.excel.repository.excel-folder", ExcelRepositoryProperties.EXCEL_DEFAULT_FOLDER);
    ...
    private void mapPropertiesSource(List<Resource> resources, ConfigurableEnvironment environment) {
        if (Objects.nonNull(resources)) {
            ...
            MapPropertySource mapPropertySource = new MapPropertySource("excelRepositoryQueries", collect);
    ...
    private void printRegistered(String name) {
        log.info("Excel Source registered: " + name);
    }
